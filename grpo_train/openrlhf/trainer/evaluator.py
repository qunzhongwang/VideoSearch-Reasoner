    ...
    def eval_unit(self, args, ep, global_step, dataloader):
        ...
        for idx, rand_prompts in enumerate(dataloader):
            ...
            for i, experience in enumerate(exp_list):
                ...
                print(f"!!!! ========== eval step {i} ==========")
                info = self.replay_buffer.eval_items[-1].info
                for k in keys:
                    infos[k].append(info[k])
                out_lens = infos['response_length']
                for k,vlist in infos.items():
                    tmpinfos[k] = np.mean(vlist)
                tmpinfos['generation_exceed_rate'] = np.mean([x>args.generate_max_len-1 for x in out_lens])
                
                # if torch.distributed.get_rank() == 0 :
                #     print(self.replay_buffer)
                #     breakpoint()
                # torch.distributed.barrier()
                tmpinfos = self.strategy.all_reduce(tmpinfos)
                if torch.distributed.get_rank() == 0 :
                    wandb.log(tmpinfos)
                    print(tmpinfos)
