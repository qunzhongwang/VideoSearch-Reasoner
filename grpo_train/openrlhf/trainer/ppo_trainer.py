...
class PPOTrainer(ABC):
        ...
        if self.strategy.args.use_wandb and self.strategy.is_rank_0():
            ...
            if args.reinit_wanb == "no":
                ...
                wandb.init(
                    ...
                    name=strategy.args.wandb_run_name,
                    config=strategy.args.__dict__,
                    id = args.reinit_wanb,  # 指定运行的 ID
                    resume="must" ,     # 必须恢复指定的运行
                    reinit=True, # TODO
                )
    ...
    def eval_unit(self, args, ep, global_step, dataloader):
        keys = ['reward', 'response_length', 'validity','match','usefmt','round1_nwait', 'round0_nwait','format_correctness']
        infos = {k:[] for k in keys}
        columns = ['Curr_Video1', 'Answer1', 'Curr_Video2', 'Answer2', 'Curr_Video3', 'Answer3', 'Curr_Video4', 'Answer4', 'Curr_Video5', 'Answer5', 'Curr_Video6', 'Answer6']
        ...
        for idx, rand_prompts in enumerate(dataloader):
            ...
            exp_list, all_answer_images_pair = self.get_explist_from_prompts(args, ep, rand_prompts, is_eval=True, eval_step=global_step)
            table = wandb.Table(columns=columns)
            for row in self._eval_table.data:
                table.add_data(*row)
            for sublist in all_answer_images_pair.values():
                row = []
                for text, image_list in sublist:
                    wandb_image_list = [wandb.Image(img) for img in image_list]
                    row.extend([wandb_image_list, text])
    ...
    def train_unit(self, args, pbar, steps, ep, eval_data=None): # for an episode
        ...
        if self._table is None:
            self._table = wandb.Table(columns=columns)
        args.steps = steps
        total_consumed = 0 
        num_real = 0 
        ...
        for idx, rand_prompts in enumerate(self.prompts_dataloader):
            ...
            eval_save = False
            if eval_data and not no_eval and ((args.eval_steps>0 and (steps)%args.eval_steps==0) or args.eval_steps<=0):
                print(f'!!!! doing evaluation @Step{steps}', eval_data)
                if eval_only:
                    tmp = eval_data 
                elif small_eval:
                    tmp = eval_data[:64]
                else: tmp = eval_data[:64]
                # make sure eval_bsz*nsample%num_vllm == 0
                
                eval_bsz = args.micro_rollout_eval_batch_size 
            ...
            exp_list, all_answer_images_pair = self.experience_maker.make_experience_list(rand_prompts, is_eval=False, eval_step=None, **self.generate_kwargs)
            ...
            print('!!!! [rbuffer] done.')
            print(f'===> [verbose] waiting for all actors, I am {torch.distributed.get_rank()}')
            torch.distributed.barrier()
        ...
        for episode in rg:
            if isinstance(self.prompts_dataloader.sampler, DistributedSampler):
                self.prompts_dataloader.sampler.set_epoch(
                    episode, consumed_samples=0 if episode > start_episode else consumed_samples # 2
                    # TODO
                )
            pbar = tqdm(
    ...
    def training_step_actor(self, experience: Experience, **kwargs) -> Dict[str, float]:
        ...
        if sft_only: loss = aux_loss
            ...
            nonzero = np.mean([torch.any(x != 0).item() for x in experience.advantages])
