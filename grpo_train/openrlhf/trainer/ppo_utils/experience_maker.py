    ...
    parameters = {
        ...
        "properties": {
            "target_frames": {
                ...
                "items": {
                    "type": "integer",
                    "description": "Frame index from 1 to 96."
                }
            }
        },
...
def execute_tool(images, rawimages, args, toolname, is_video, function=None):
    if toolname=='select_frames':
        ...
        if len(tgt)>8:
            ...
            if do_controlled_rectify and np.random.uniform()<0.75:
                if np.random.uniform()<0.25:
                    ...
                    tgt = sorted(tgt)
                selected_frames = function(rawimages[0], tgt)
                message = tgt
            else: 
                selected_frames = []
            # selected_frames = function(images[0], [x-1 for x in tgt][::2]) # video is always in the first item
        elif max(tgt)>len(rawimages[0]):
            message = f"There are {len(rawimages[0])} frames numbered in range [1,{len(rawimages[0])}]. Your selection is out of range."
            selected_frames = []
        else:
            message = ""
            candidates = rawimages[0]
        ...
        return [], "Incorrect Tool. You can not use crop for video here"
...
DEFAULT_MAX_PIXELS = int(os.getenv("MAX_PIXELS", 5120*28*28*4))
IMAGE_FACTOR = 28
...
class RemoteExperienceMaker(NaiveExperienceMaker):
    def __init__(self, *args, vllm_engines: List = None, packing_samples=False, **kwargs):
        ...
        ####### new version
        self.operations = dict(select_frames=SelectFrames()) #crop_image_normalized=SelectFrames(), 
        notool = getattr(self.strategy.args, "system_prompt", "none")=="notool"
        
        self.tools = [] if notool else [self.operations[k].function for k in ['select_frames']] #'crop_image_normalized', 
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], is_eval=False, **generate_kwargs) -> List[Experience]:
        ...
        if ('use_response' in data_version) and not is_eval:
            samples_list, all_answer_images_pair = self.generate_samples(all_prompts, use_response=True, **generate_kwargs)
        else:
            samples_list, all_answer_images_pair = self.generate_samples(all_prompts, **generate_kwargs)
        torch.distributed.barrier()
        def gather_and_distribute_samples(samples_list, world_size, rank, args):
            """
            Gather `samples_list` from all processes, then evenly redistribute the samples among all processes.
            Drops extra samples to ensure equal distribution.
            
            Args:
                samples_list (list): The list of samples from the current process.
                world_size (int): Total number of processes.
                rank (int): Current process rank.

            Returns:
                list: The evenly distributed samples for this process.
            """
            # Step 1: Gather all `samples_list` from all processes
            grad_accum = ( args.train_batch_size // world_size) // args.micro_train_batch_size

            my_count = len(samples_list)
            gathered_samples_count = [None for _ in range(world_size)]
            print("[DEBUG] I am doing gathering before. May error")
            
            torch.distributed.all_gather_object(gathered_samples_count, my_count)
            count_base = min(gathered_samples_count)
            count_total = sum(gathered_samples_count)

            print(f"total {count_total}, now {( count_total // (world_size * grad_accum) )* grad_accum * world_size}")

            count_antipated = ( count_total // (world_size * grad_accum) )* grad_accum

            base_list = samples_list[:count_base]

            # **Step 1: Adjust gathered_samples_count to ensure total samples <= 24**
            if (count_total - world_size * count_base) > 32:
                # Calculate the overflow (how many samples need to be removed)
                overflow = (count_total - world_size * count_base) - 32

                # Sort gathered_samples_count (index + value) to target the largest values for reduction
                ranked_counts = sorted(
                    [(i, gathered_samples_count[i] - count_base) for i in range(world_size)],
                    key=lambda x: x[1],  # Sort by the extra part (count - base)
                    reverse=True
                )

                # Reduce the extra samples starting from the largest
                for i, extra in ranked_counts:
                    if overflow <= 0:
                        break
                    reduce_by = min(overflow, extra)
                    gathered_samples_count[i] -= reduce_by
                    overflow -= reduce_by
            
            count_antipated = ( sum(gathered_samples_count) // (world_size * grad_accum) )* grad_accum

            print(f"total {count_total}, now { count_antipated * world_size}")

            
            print("[DEBUG] I am doing gathering. May error")
            gathered_samples = [None for _ in range(world_size)]  # Placeholder for gathered data
            torch.distributed.all_gather_object(gathered_samples, samples_list[count_base:gathered_samples_count[rank]])

            print("[DEBUG] I am done gathering")

            # Step 2: Flatten the gathered data into a single list
            rest_samples = []
            for samples in gathered_samples:
                rest_samples.extend(samples)
            
            # Step 3: Calculate the total number of samples and drop extra samples
            count_rest = len(rest_samples)
            # samples_per_rank = ( total_samples // (world_size * grad_accum) )* grad_accum# Ensure equal division by dropping last

            rest_samples = rest_samples[:(count_antipated - count_base) * world_size]  # Drop extra samples

            

            # Step 4: Redistribute samples among processes
            start_idx = rank * (count_antipated - count_base) 
            end_idx = start_idx + (count_antipated - count_base)
            added_list = rest_samples[start_idx:end_idx]
            del rest_samples
            torch.cuda.empty_cache()
            return base_list + added_list
        
        if not is_eval:
            samples_list = gather_and_distribute_samples(samples_list, world_size=torch.distributed.get_world_size(), rank=torch.distributed.get_rank(), args = args)
        ...
        experiences, rewards = self.handle_advantages(experiences, nsample=len(experiences))
        ...
        return experiences, all_answer_images_pair
    ...
    def generate_samples(self, all_prompts: List[Dict], **generate_kwargs) -> List[Samples]:
        ...
        samples, all_answer_images_pair = self._generate_vllm(all_prompts, **generate_kwargs)
        ...
        if self.strategy.args.vllm_enable_sleep:
            if torch.distributed.get_rank() == 0:
                ...
                ray.get(refs)
        return samples, all_answer_images_pair
    ...
    def get_logprobs_and_logs(self, batched_sample: Samples, is_eval=False, validity=None, ) -> Experience:
        ...
        format_correctness = batched_sample.format_correctness
        ...
        # init log probs
        # print("?????")
        # print(visual_inputs_cpu)

        # for k, v in  visual_inputs_cpu.items():
        #     print(k)
        #     print(v)
        #     print("***")
        ...
        if is_eval or use_response:
            ...
            if base_action_log_probs is not None:
                base_action_log_probs = base_action_log_probs.to('cpu')
    ...
    def _generate_vllm(self, all_prompts: List[str], use_response=False, skip_generation=False, **kwargs) -> List[Samples]:
        ...
        max_imgnum = 64 # 24 if is_eval else 16
        ...
        all_images = dict()
        all_sliced_images = dict()
        all_answer_images_pair = dict()
        first_raw_images = dict()
        all_raw_images = dict()
        all_raw_images = dict()
        nsample = 1 if is_eval else args.n_samples_per_prompt
        ...
        if is_eval or not skip_generation:
            ...
            for i, llm in enumerate(llms):
                ...
                for idx, (uuid, conv, imglist, video_flag, pp) in enumerate(zip(batch_uids, conversations, images, has_video, prompts)):
                    ...
                    if video_flag:
                        ...
                        selected_frames = imglist[::step][:8]  
                        
                        conversations[idx][1]['content'][0]['video'] = conversations[idx][1]['content'][0]['video'][::step][:8]
                        
                        prompts[idx] = self.data_processor.processor.apply_chat_template(conversation = conversations[idx])
                        
                        rawimagelist, images[idx], all_images[uuid],all_sliced_images[uuid]= [imglist], selected_frames, [selected_frames],[[selected_frames]]
                        first_raw_images[uuid] = [selected_frames]
                    else: 
        ...
        solutions_round0 = [sol.strip() for sol in solutions_round0]
        solutions_round0 = [sol.replace("</tool_call>\n<|im_end|>","</tool_call>") for sol in solutions_round0]
        solutions_round0 = [sol.replace("</tool_call>\n\\boxed{2}<|im_end|>","</tool_call>") for sol in solutions_round0]
        solutions_round0 = [sol.replace("</tool_call>\n\n\\boxed{2}<|im_end|>","</tool_call>") for sol in solutions_round0]
        solutions_round0 = [sol.replace("</tool_call>\n\\boxed{1}<|im_end|>","</tool_call>") for sol in solutions_round0]
        solutions_round0 = [sol.replace("</tool_call>\n\n\\boxed{1}<|im_end|>","</tool_call>") for sol in solutions_round0]
        for idx, sol in enumerate(solutions_round0):
            if "</tool_call>" in sol:
                iiidx = sol.find("</tool_call>")
                solutions_round0[idx] = sol[:iiidx + len("</tool_call>")]

        # all_answer_images_pair[idx2uid[i for len(idx2uid)]] = [(solutions_round0[i], all_sliced_images[idx2uid[i]][0][0])]
        for i in range(len(idx2uid)):
            uid = idx2uid[i]
            value = [(solutions_round0[i], all_sliced_images[uid][0][0])]
            all_answer_images_pair[uid] = value

        print("*"*80)
        print("important "*10)
        print(solutions_round0[i])
        print("*"*80)
        ...
        all_a_texts = [x for x in solutions_round0]
        num_toolcalls = [0] * len(all_qa_texts)
        num_format = [1] * len(all_qa_texts)
        num_toolfails = [0] * len(all_qa_texts)
        
        ####### add saturation calculation here
        tool_end = "</tool_call>"
        maxtool = getattr(args, "maxturn", 4)
        ...
        def process(input_text):
            """
            Process the input text to extract and reconstruct sections with proper tags using `find()`.
            Args:
                input_text (str): The raw input string to process.
            Returns:
                str: A reconstructed string with proper tags, or an empty string if processing fails.
            """
            # Helper function to find a substring (case-insensitive)
        
            okay_ = 0

            set1 = ["synthesis", "think", "recommend answer"]
            set2 = ["synthesis", "think", "recommended answer"]
            set3 = ["synthesis", "think", "final answer"]
            sets = [set1, set2, set3]
            new_string = input_text
            for _set in sets:
                flag = 1
                for tag in _set:
                    if tag not in input_text.lower():
                        flag = 0
                if flag:
                    okay_ = 1
                    if "</synthesis>" in input_text:
                        input_text = input_text.replace("</synthesis>", "")
                    if "</think>" in input_text:
                        input_text = input_text.replace("</think>", "")
                    pos_ = [ (input_text.lower().find(tag),input_text.lower().find(tag) + len(tag))  for tag in _set]

                    new_string = "<synthesis>" + input_text[pos_[0][1]:pos_[1][0]].strip(" \n\b#<>") +  "</synthesis>" + "\n" + \
                        "<think>" + input_text[pos_[1][1]:pos_[2][0]].strip(" \n\b#<>") + "</think>"
                    if "final answer" in _set:
                        if "</final answer>" in input_text:
                            input_text = input_text.split("</final answer>")[0]
                        new_string = new_string + "<final answer>" + input_text[pos_[2][1]:].strip(" \n\b#<>") + "</final answer>"
                    else:
                        if "</recommend answer>" in input_text:
                            input_text = input_text.replace("</recommend answer>", "")
                        new_pos = input_text.lower().find("<tool_call>")
                        if new_pos == -1:
                            return new_string, 0
                        new_string = new_string + "<recommend answer>" + input_text[pos_[2][1]:new_pos].strip(" \n\b#<>") + "</recommend answer>" + input_text[new_pos:]
                    break
            return new_string, okay_
        ...
        while True: 
            ...
            for out_idx, (qqid, out, qatext,atext, fflag) in enumerate(zip(qids_expanded, all_outputs, all_qa_texts, all_a_texts,all_flags)):
                if fflag: continue 
                ...
                rsp, fffflag = process(rsp)

                
                # rsp.endswith(tool_end) also fine
                tags = ["<synthesis>", "</synthesis>", "<think>", "</think>"]

                # 检查 rsp 中每个 tag 恰好出现一次，并且按顺序出现
                if all(rsp.count(tag) == 1 for tag in tags):  # 每个 tag 恰好出现一次
                    # 获取 tag 在 rsp 中的索引，并检查是否按顺序出现
                    tag_indices = [rsp.find(tag) for tag in tags]
                    if tag_indices == sorted(tag_indices):  # 检查索引是否递增
                        num_format[out_idx] = num_format[out_idx]
                    else:
                        num_format[out_idx] = 0
                else:
                    num_format[out_idx] = 0
                ...
                if "<synthesis>" in rsp:
                    index = rsp.find("<synthesis>")
                    rsp = rsp[index:]
                
                if "<tool_call>" in rsp:
                    if not (( "<recommend answer>" in rsp) and (not "</recommend answer>" in rsp)):
                        num_format[out_idx] = 0
                else:
                    if not (("<final answer>" in rsp) and ("</final answer>" in rsp)):
                        num_format[out_idx] = 0
                

                msg_this = [dict(role='assistant', content=[
                    dict(type='text', text=rsp),])]
                msg_old = [dict(role='assistant', content=[
                    dict(type='text', text=old_rsp),])]

                # 检查 <|im_start|>assistant 的数目
                if rsp.count("<|im_start|>assistant") > 2:  # 最多只能有 1 个
                    num_format[out_idx] = 0

                # 检查 assistant 的数目
                if rsp.count("assistant") > 5:  # 最多只能有 5 个
                    num_format[out_idx] = 0
                
                # 新增条件：检查 \\boxed{1} 或 \\boxed{2}
                if ("\\boxed{1}" in rsp or "\\boxed{2}" in rsp) or True:
                    # 确保 \\boxed{1} 或 \\boxed{2} 出现在 "</think>" 之后
                    think_pos = rsp.find("</think>")
                    boxed_1_pos = rsp.find("\\boxed{1}")
                    boxed_2_pos = rsp.find("\\boxed{2}")

                    # 获取 \\boxed{1} 或 \\boxed{2} 的位置和内容
                    boxed_pos = max(boxed_1_pos, boxed_2_pos)  # 取较大的索引值
                    boxed_str = "\\boxed{1}" if boxed_1_pos != -1 else "\\boxed{2}"

                    # 检查条件
                    if (not rsp.endswith(boxed_str)): #(not rsp.endswith("</think>")) and 
                        num_format[out_idx] = 0
                    
                    ans_1_count = rsp.count("\\boxed{1}")
                    ans_2_count = rsp.count("\\boxed{2}")
                    if ans_1_count + ans_2_count > 1:
                        num_format[out_idx] = 0
                    
                # if (not rsp.startswith("<manuscript>")):
                #     num_format[out_idx] = 0
                num_format[out_idx] = num_format[out_idx] or fffflag
                ...
                force_terminate = (num_toolcalls[out_idx] >= args.max_chain_length  or len(all_images[uuid]) > max_imgnum or cur_tokens>12*1024-200)
                force_terminate = force_terminate or not num_format[out_idx]
                ...
                if out_idx == 0:
                    ...
                    print("*"*60)
                    print(f"++++ [debug] qqid={qqid}, iter={niter}, A={msg_old}, require_tool={require_tool}")
                    # if niter==2:
                if finish_flag: 
                    all_conversations[uuid] = all_conversations[uuid] + msg_this
                    if do_dump and num_format[out_idx]:
                        json.dump([all_conversations[uuid],
                            all_vllm_inputs[uuid]['prompt'],
                            str(all_images[uuid])
                            ], open(f"{tfolder}/conv.json",'w'),indent=4, ensure_ascii=False)
                    continue 
                ...
                try:
                    ...
                    if tool_name=='select_frames': 
                        ...
                        if len(selected_frames) > 8:
                            print("[!!!!] here over long")
                            selected_frames = selected_frames[:8]
                        ...
                        if len(selected_frames)==0:
                            print("[!!!!] here error process")
                            
                            msg_this.append(
                                dict(role='user', content=[
                                    dict(type='text', text="After selecting frames:" + f"\n{info}"),
                                ] )
                            )
                        else:    
                            msg_this.append(
                            dict(role='user', content=[
                                # dict(type='text', text="\nHere are the selected frames (Frame Size: {}x{})".format(added[0].size[0], added[0].size[1])), #, Numbered {} to {}):".format(added[0].size[0], added[0].size[1], len(imagelist), len(selected_frames)+len(imagelist)-1)),
                                dict(type='text', text="\nAfter selecting frames: Here are the selected frames"), #(Frame Size: {}x{})".format(added[0].size[0], added[0].size[1])), #, Numbered {} to {}):".format(added[0].size[0], added[0].size[1], len(imagelist), len(selected_frames)+len(imagelist)-1)),
                            ] + [dict(type='image', image=targetpath) for _ in range(len(selected_frames))])
                            )
                        
                    else:
                        print("[maynver happen!!!!!!! crop op!]")
                        ...
                        msg_this.append(
                            dict(role='user', content=[
                                ...
                                dict(type='text', text="After selecting frames:"+"\nHere is the cropped image"),
                    ...
                    msg_this.append(
                        dict(role='user', content=[
                            dict(type='text', text=f"After selecting frames:" + "\nExecution error:\n{error_info}\n")
                        ])
                    )
                ...
                this_images = first_raw_images[uuid] + added
                ...
                all_sliced_images[uuid].append([all_sliced_images[uuid][0][0], added]) 
                ...
                txt_list = prompts.split("After selecting frames:")
                if len(txt_list) > 2:
                    for idx, conv in enumerate(txt_list[1:-1]):
                        txt_list[idx + 1] = conv.replace("<|image_pad|>", "").replace("<|vision_end|>", "").replace("<|vision_start|>", "")
                    prompts = "After selecting frames:".join(txt_list)
                prompts.replace("<|im_start|>assistant\\n<|im_start|>assistant", "<|im_start|>assistant")
                prompts.replace("<|im_start|>assistant\n<|im_start|>assistant", "<|im_start|>assistant")

                all_vllm_inputs[uuid]['prompt'] = prompts
                
                if video_flag:
                    all_vllm_inputs[uuid]["multi_modal_data"]['image'] = this_images[1:]
                else:
            ...
            for iidx, i in enumerate(req_indexlist):
                uid = idx2uid[i]
                value = (new_texts_out[iidx], all_sliced_images[uid][-1][-1])
                all_answer_images_pair[uid].append(value)
        ...
        # 在 barrier 前后打印日志
        print(f"[{get_timestamp()}] Rank {rank} reached barrier.")
        torch.distributed.barrier()
        print(f"[{get_timestamp()}] Rank {rank} left barrier.")
        ...
        for idx in range(0, len(rets_round1), nsample):
            ...
            for mres, ncall in zip(correctness, ntoolcalls):
                if mres>0.5 and ncall<0.1: 
                    ...
                    break 
            rapr = np.mean([ncall>1. for ncall in ntoolcalls])
            nfmt = num_format[idx:idx+nsample]
            ...
            if all(corr == 1 for corr in correctness):
                # print("All correct, so not keeping")
                should_keep = [0] * nsample
            elif all(corr != 1 for corr in correctness):  # 全为 0 或 -1
                # print("All wrong, so not keeping")
                should_keep = [0] * nsample
            else:
                # print("Mixed correctness, so keeping")
                should_keep = [1] * nsample
            
            #TODO
            should_keep = [1] * nsample

            for iidx, (mres, ncall, isvideo, ffmt) in enumerate(zip(correctness, ntoolcalls,videoflags, nfmt)):
                this_r = float(mres)
                ...
                curiosity = 0.0 
                penalty = 0.0
                step = None if not hasattr(args, 'steps') else args.steps
                coeff = 1
                if step :
                    if step >= 30:
                        coeff = 0.2
                ...
                if isvideo and ncall>0.1: # for video
                    if rapr < 0.6:
                        curiosity = (rapr - 0.6) * 1 * coeff # 奖励随着 rapr 增大而减小，最高为 0.15
                    else:
                        curiosity = 0.0  # rapr >= 0.3 时没有额外奖励

                    # 添加 penalty 逻辑
                    if ncall >= 4:
                        penalty = 0.5 * 3 * coeff # 固定最大惩罚
                    elif ncall <= 1:
                        penalty = -1 * coeff
                    else:
                        penalty = 0.5 * (ncall - 1) * coeff  # 根据调用次数调整惩罚
                    ...
                    if rapr < 0.5:
                        curiosity = (rapr - 0.5) * 0.15  # 奖励随着 rapr 增大而减小，最高为 0.15
                    else:
                        curiosity = 0.0  # rapr >= 0.3 时没有额外奖励

                    # 添加 penalty 逻辑
                    if ncall >= 4:
                        penalty = 0.05 * 2  # 固定最大惩罚
                    else:
                        penalty = 0.05 * (ncall - 1)  # 根据调用次数调整惩罚
            ...
            std_rewards = np.std(this_rewards)
            is_uniform = False
        ...
        #print(f"!!!![verbose]{all_conversations}")
        ...
        if not is_eval:
            for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
                ...
                batch_format = num_format[i : i + self.strategy.args.micro_rollout_batch_size]
                ...
                batch_sliced_images = [all_sliced_images[uuid] for uuid in  batch_uuids]
                ...
                should_keep = total_should_keeps[i : i + self.strategy.args.micro_rollout_batch_size]
                ...
                batch_list_conv = []
                batch_list_idx = []
                for conv in batch_conv:
                    new_list = []
                    idx = 0
                    list_idx = []
                    for iid in range(3,len(conv) + 1,2):
                        # if iid >= 7:

                        new_list.append(conv[:iid])
                        idx = idx + 1
                        list_idx.append(idx)
                    list_idx = [iidx-idx for iidx in list_idx]
                    batch_list_idx.append(list_idx)
                    batch_list_conv.append(new_list)

                batch_list_texts = []
                batch_ans_texts = []
                for list_conv in batch_list_conv:
                    new_list_texts = []
                    new_ans_texts = []
                    for conv in list_conv:
                        new_list_texts.append(
                            self.data_processor.processor.apply_chat_template(conv[:-1], tokenize=False, add_generation_prompt=False)
                            )
                        new_ans_texts.append(
                            self.data_processor.processor.apply_chat_template([conv[-1]], tokenize=False, add_generation_prompt=False)
                            )
                    batch_list_texts.append(new_list_texts)
                    batch_ans_texts.append(new_ans_texts)
                
                for list_idx, (list_texts, ans_texts) in enumerate(zip(batch_list_texts, batch_ans_texts)):  # 遍历外层列表

                    for text_idx, (texts, ans) in enumerate(zip(list_texts, ans_texts)):  # 遍历内层列表
                        txt_list = texts.split("After selecting frames:")
                        if len(txt_list) > 2:
                            for idx, conv in enumerate(txt_list[1:-1]):  
                                # 对每个分割后的文本进行替换操作
                                txt_list[idx + 1] = conv.replace("<|image_pad|>", "< No Focus >").replace("<|vision_end|>", "").replace("<|vision_start|>", "")

                            # 将修改后的列表重新组合为一个字符串
                            texts = "After selecting frames:".join(txt_list)

                        # 进行额外替换和去除多余空格的操作
                        texts = texts.replace("<|im_start|>assistant\\n<|im_start|>assistant", "<|im_start|>assistant")
                        texts = texts.strip()

                        import re
                        num_of_start = ans.count("<|im_start|>assistant")
                        if num_of_start > 1:
                            # 保留第一个，删除其他
                            ans = re.sub(r'(<\|im_start\|>assistant)', lambda m, c=[0]: m.group(0) if (c.append(1) or c[0] == 0) else '', ans)
                        if "<manuscript>" in ans:
                            ans = "<|im_start|>assistant\n" + ans.split("<manuscript>")[-1]
                        else:
                            ans = "<|im_start|>assistant\n" + ans.split("<|im_start|>assistant\n")[-1]
                        # print(ans)
                        # 将修改后的文本重新存回原位置
                        batch_list_texts[list_idx][text_idx] = texts
                        batch_ans_texts[list_idx][text_idx] = ans
                
                # print(batch_ans_texts)
                ...
                padded = False 
                if args.collect_method == "all":
                    pass
                elif args.collect_method == "nofirst":
                    if len(batch_list_texts[0]) == 1:
                        batch_list_texts[0] = batch_list_texts[0]
                        batch_sliced_images[0] = batch_sliced_images[0]
                        batch_list_idx[0] = batch_list_idx[0]
                        batch_ans_texts[0] = batch_ans_texts[0]
                    else:
                        batch_list_texts[0] = batch_list_texts[0][1:]
                        batch_sliced_images[0] = batch_sliced_images[0][1:]
                        batch_list_idx[0] = batch_list_idx[0][1:]
                        batch_ans_texts[0] = batch_ans_texts[0][1:]
                elif args.collect_method == "last":
                    batch_list_texts[0] = [batch_list_texts[0][-1]]
                    batch_sliced_images[0] = [batch_sliced_images[0][-1]]
                    batch_list_idx[0] = [batch_list_idx[0][-1]]
                    batch_ans_texts[0] = [batch_ans_texts[0][-1]]
                
                for idx,( _new_text, this_visual, to_end, ans) in enumerate(zip(batch_list_texts[0],batch_sliced_images[0], batch_list_idx[0], batch_ans_texts[0])):
                    sk = should_keep[0]
                    if not sk:
                        print("*"*20)
                        print("due to all true or all flase")
                        print("*"*20)
                        continue
                    # print(ans)
                    # print(batch_ans_texts[0])
                    batch_s = []
                    if video_flag:
                        video = this_visual[0]
                        if len(this_visual) > 1:
                        ...
                        if len(imagelist)==0:
                            ...
                            new_text = _new_text + "<|vision_start|><|image_pad|><|vision_end|>"
                            padded = True 
                        else:
                            this_imglist = imagelist
                            new_text = _new_text
                        
                        num_video_pad = new_text.count("<|video_pad|>")
                        num_image_pad = new_text.count("<|image_pad|>")

                        ffflag = 0
                        if num_video_pad >= 1 and num_image_pad == len(this_imglist):
                            ffflag = 1
                            print("[no probelm] same len")
                        if ffflag == 0:
                            print("[!!!!! verbose] do not match! need speciail attention! ")
                            print(_new_text)
                            print(f"expected length {len(this_imglist)}, but {num_image_pad}")
                            print("full text")
                            print(batch_list_texts[0][-1])
                            continue
                        ...
                        q_visual_inputs = self.data_processor.processor(
                            text=[new_text],
                            images=[this_imglist],
                            videos=[video],
                            padding=True,
                    ...
                    q_seqlist = q_visual_inputs['input_ids']
                    visual_inputs.pop('input_ids')
                    visual_inputs.pop('attention_mask')

                    # 分割 seqlist：Q 和 A 的 token
                    q_tokens = seqlist[0][:len(q_seqlist[0]) + 2]  # Q 部分，包括 <|im_start|>assistant
                    a_tokens = seqlist[0][len(q_seqlist[0]) + 2:]  # A 部分，不包括 <|im_start|>assistant

                    # 使用 data_processor.processor 将 token 转化为文本
                    q_text = self.data_processor.processor.tokenizer.decode(q_tokens, skip_special_tokens=False)
                    a_text = self.data_processor.processor.tokenizer.decode(a_tokens, skip_special_tokens=False)
                    all_text = self.data_processor.processor.tokenizer.decode(seqlist[0], skip_special_tokens=False)
                    # 验证
                    # 打印调试信息
                    # print("Q Text:\n", q_text)
                    # print("A Text:\n", a_text)
                    ...
                    # 验证 <|im_start|>assistant 是否正确分割
                    assistance_tok = self.data_processor.processor.tokenizer(
                        "<|im_start|>assistant", add_special_tokens=False
                    )["input_ids"]

                    # print("Assistance Token IDs:", assistance_tok)
                    # print("Tokens around split point:", seqlist[0][len(q_seqlist[0]) - 5:len(q_seqlist[0]) + 5])
                    # print("Tokens qianhou split point:", seqlist[0][len(q_seqlist[0]) - 1:len(q_seqlist[0]) + 1])
                    ...
                    for seq, single_conv in zip(seqlist,[batch_list_conv[0][idx]]):
                        segments = []
                        segment = []
                        tmp_texts = []
                        info = []
                        ...
                        for idx, (q,a) in enumerate(segments): 
                            # lth = len()
                            qtext = self.data_processor.processor.apply_chat_template(
                                    q, tokenize=False, add_generation_prompt=True 
                            )
                            # if idx > 1 and idx < lth:
                            #     qtext = qtext.replace("<|image_pad|>", "< No Focus >").replace("<|vision_end|>", "").replace("<|vision_start|>", "")
                            ...
                            atext = atext.replace("<|im_start|>assistant\\n<|im_start|>assistant", "<|im_start|>assistant")
                            mlist.append(atext)
                        ...
                        #     tmp_texts = []
                        #     info = []
                        # print(f"!!!![verbose] [TEXT]{tmp_texts}")
                        # print(f"!!!![verbose] [INFO]{info}")
                        if tmp_texts == []:
                            # print(f"!!!![verbose] [WARN]{tmp_texts}")
                            samples_list.append(
                                self._backupsample
                            )
                            # print(f"!!!![verbose] [SAMPLE]{samples_list[-1]}")
                            self._flag = 1
                            continue
                        ...
                        q_tokens.append(seqlist[0][ : len(q_seqlist[0]) + 2])
                        a_tokens.append(seqlist[0][len(q_seqlist[0]) + 2:])
                    ...
                    if self._flag == 1:
                        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        print(f"!!!! [WARNING]  generated [] at {i} part")
                        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        self._flag = 0
                        continue
                    ...
                    for idx, (qtoken, atoken) in enumerate(zip(q_tokens, a_tokens)):
                        print(f"lenth of q {len(qtoken)} ,a {len(atoken)}")
                        print(f"qtoken -2 {qtoken[-2]} is 151644")
                    ...
                    if sum(expected_vidpad_list+expected_imgpad_list) != ntokens[0]:
                        print("fatal ! fatal !fatal ! fatal !fatal ! fatal !fatal ! fatal !fatal ! fatal ! not same length!")
                        continue
                    ...
                    print(f"we now have action lth!!!!! {len(action_mask[0])}")
                    ...
                    samples_list.append(
                        Samples(
                            ...
                            format_correctness=batch_format,
                    ...
                    if i == 0:
                        self._backupsample = deepcopy(samples_list[-1])
                    if self._cnt == 0:
                        self._cnt += 1

            # import random
            # num_of_actors = args.actor_num_nodes * args.actor_num_gpus_per_node
            # n = (len(samples_list) // num_of_actors) * num_of_actors
            # samples_list = random.sample(samples_list, n)
            return samples_list, all_answer_images_pair
            ...
            for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
                ...
                ###################
                assert is_eval, "mix of video and image only support mrbsz==1"
                video_flag = batch_flags[0]
                padded = False 
                if video_flag:
